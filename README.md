...
* [How to install](#how-to-install)
  * [1. Install dependencies](#1-install-dependencies)
    * [1.1 Custom Terraform binaries and OpenTofu support](#REDACTED)
  * [2. Install the pre-commit hook globally](#REDACTED)
  * [3. Add configs and hooks](#3-add-configs-and-hooks)
  * [4. Run](#4-run)
* [Available Hooks](#available-hooks)
* [Hooks usage notes and examples](#hooks-usage-notes-and-examples)
  * [Known limitations](#known-limitations)
  * [All hooks: Usage of environment variables in `--args`](#REDACTED)
  * [All hooks: Usage of `__GIT_WORKING_DIR__` placeholder in `--args`](#REDACTED)
  * [All hooks: Set env vars inside hook at runtime](#REDACTED)
  ...
  * [checkov (deprecated) and terraform\_checkov](#REDACTED)
  * [infracost\_breakdown](#infracost_breakdown)
  * [terraform\_docs](#terraform_docs)
  * [terraform\_docs\_replace (deprecated)](#REDACTED)
  ...
  * [terraform\_wrapper\_module\_for\_each](#REDACTED)
  * [terrascan](#terrascan)
  * [tfupdate](#tfupdate)
...
* [Docker Usage](#docker-usage)
  ...
  * [Download Terraform modules from private GitHub repositories](#REDACTED)
* [GitHub Actions](#github-actions)
* [Authors](#authors)
* [License](#license)
  * [Additional information for users from Russia and Belarus](#REDACTED)
...
> **NOTE**
> The build install scripts are calling the GitHub API to resolve the release URL. If you need to authenticate those calls, you can pass a GitHub token (the `GITHUB_TOKEN` environment variable is expected to be set with an [access token](https://docs.github.com/en/authentication/REDACTED/REDACTED)):
> ```bash
...
Otherwise, you can follow [this gist](https://gist.github.com/etiennejeanneaurevolve/REDACTED):
...
* [`hcledit`][hcledit repo] required for `REDACTED` hook
...
you installed the pre-commit hook globally ([step 2](#REDACTED)),
...
| `checkov` and `terraform_checkov`                      | [checkov][checkov repo] static analysis of terraform templates to spot potential security issues. [Hook notes](#REDACTED)                                                                        | `checkov`<br>Ubuntu deps: `python3`, `python3-pip`                                   |
...
| `terraform_docs_replace`                               | Runs `terraform-docs` and pipes the output directly to README.md. **DEPRECATED**, see [#248](https://github.com/antonbabenko/pre-commit-terraform/issues/248). [Hook notes](#REDACTED)                  | `python3`, `terraform-docs`                                                          |
...
| `REDACTED`                    | Generates Terraform wrappers with `for_each` in module. [Hook notes](#REDACTED)                                                                                                                         | `hcledit`                                                                            |
...
> All, except deprecated hooks: `checkov`, `terraform_docs_replace` and hooks which can't be paralleled this way: `infracost_breakdown`, `REDACTED`.  
...
1. `terraform_docs` and `REDACTED` will insert/update documentation generated by [terraform-docs][terraform-docs repo] framed by markers:
...
2. It is possible to pass additional arguments to shell scripts when using `terraform_docs` and `REDACTED`.
...
> ```
>
> Why? When v2.x will be introduced - the default mode will be changed, probably, to `REDACTED`.
>
> You can check available modes for hook below.
> </details>


1. The hook can work in a few different modes: `REDACTED` with and without [terraform_validate hook](#terraform_validate) and `always-regenerate-lockfile` - only with terraform_validate hook.

    * `REDACTED` without terraform_validate - only checks that lockfile has all required SHAs for all providers already added to lockfile.

        ```yaml
        - id: terraform_providers_lock
          args:
          - --hook-config=--mode=REDACTED
        ```

    * `REDACTED` with [terraform_validate hook](#terraform_validate) - make up-to-date lockfile by adding/removing providers and only then check that lockfile has all required SHAs.
     ...
     - id: terraform_tfsec
       args:
         - >
           --args=--format json
           --no-color
           -e aws-s3-enable-bucket-logging,REDACTED
    ```
...
4. `terraform_validate` in a repo with Terraform module, written using Terraform 0.15+ and which uses provider `configuration_aliases` ([Provider Aliases Within Modules](https://www.terraform.io/language/modules/develop/providers#provider-aliases-within-modules)), errors out.
   ...
   Alternately, you can use [terraform-config-inspect](https://github.com/hashicorp/terraform-config-inspect) and use a variant of [this script](https://github.com/bendrucker/REDACTED/blob/main/providers.sh) to generate a providers file at runtime:
...
`REDACTED` generates module wrappers for Terraform modules (useful for Terragrunt where `for_each` is not supported). When using this hook without arguments it will create wrappers for the root module and all modules available in "modules" directory.
...
The `REDACTED` hook attempts to determine the module's short name to be inserted into the generated `README.md` files for the `source` URLs. Since the container uses a bind mount at a static location, it can cause this short name to be incorrect.  
...
If you use a private Git repository as your Terraform module source, you are required to authenticate to GitHub using a [Personal Access Token](https://docs.github.com/en/authentication/REDACTED/REDACTED).
...
This repository is managed by [Anton Babenko](https://github.com/antonbabenko) with help from these awesome :

<a href="https://github.com/antonbabenko/pre-commit-terraform/graphs/">
  <img alt="" src="https://contrib.rocks/image?repo=antonbabenko/pre-commit-terraform" />
</a>
...
* Russia has [illegally annexed Crimea in 2014](https://en.wikipedia.org/wiki/REDACTED) and [brought the war in Donbas](https://en.wikipedia.org/wiki/War_in_Donbas) followed by [full-scale invasion of Ukraine in 2022](https://en.wikipedia.org/wiki/REDACTED).
