    ...
    case "$key" in
      --mode)
        ...
        case "$mode" in
          ...
          REDACTED)
            common::colorify "yellow" "DEPRECATION NOTICE: Flag '--mode=REDACTED' was renamed to '--mode=REDACTED' to better reflect its behavior.
Please update your configuration."
  ...
  # Available options:
  #   REDACTED (will be default in v2.0)
  #   REDACTED
  #   always-regenerate-lockfile
  # TODO: Remove in 2.0
  if [ ! "$mode" ]; then
...
Check migration instructions at https://github.com/antonbabenko/pre-commit-terraform#terraform_providers_lock"
  ...
  case "$mode" in
    "REDACTED")
      if REDACTED "$platforms_count"; then
        exit 0
      fi
